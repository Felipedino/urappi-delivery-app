...
def accepted_order(request):
    ...
    try:
        ...
        if action == "accept":
            ...
            order.status = 2  # Estado aceptado

            # Transferir los UPuntos de productos a la tienda si no se ha hecho ya
            if not order.shop_paid:
                # Encontrar al dueño de la tienda
                shop_owner = ShopOwner.objects.get(shop=order.shop)
                shop_owner_user = shop_owner.owner

                # Transferir UPuntos
                shop_owner_user.upuntos += order.products_total
                shop_owner_user.save()
    ...
    product_list = [
        {
            ...
            "prodImage": "/media/" + str(item.product.prodImage),
...
def estado_order(request, order_id):
    order = get_object_or_404(Order, id=order_id)  #
    context = {"usuario": request.user, "order": order}
...
def delivery_action(request):
    ...
    try:
        ...
        if action == "accept":
            if order.status != 2:
                messages.error(request, "Este pedido no está en estado 'en camino'")
                return redirect("app_repartidor:repartidor_perfil")
            ...
            # Transferir UPuntos al repartidor si no se ha hecho
            if not order.deliverer_paid and order.deliverer:
                # Transferir UPuntos de envío
                order.deliverer.upuntos += order.delivery_fee
                order.deliverer.save()
        ...
        # Si no entrega, estatus del pedido vuelve a pendiente
        elif action == "reject":
            if order.status != 2:
                messages.error(request, "Este pedido no está en estado 'en camino'")
                return redirect("app_repartidor:repartidor_perfil")

            # Devolver el estado a pendiente y quitar asignación
            order.status = 1  # Estado pendiente
